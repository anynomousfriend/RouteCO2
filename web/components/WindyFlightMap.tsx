        ...
        liveFlights.forEach((flight) => {
          ...
          marker.bindTooltip(
            `<div style="font-family: monospace; font-size: 11px; padding: 2px 4px;">
              ...
              <span style="color: #94A3B8; margin-left: 4px;">${Math.round((flight.baroAltitudeMeters || 0) * 3.28084)} ft</span>
            </div>`,
      ...
      .then((L) => {
        ...
        if (validTrack.length > 0) {
          ...
          const tdIcon = L.divIcon({
            html: `
              <div style="display: flex; align-items: center; justify-content: center; width: 32px; height: 32px;">
                <span style="position: absolute; width: 28px; height: 28px; border-radius: 50%; background: rgba(224, 90, 71, 0.25); animation: ping 2s cubic-bezier(0, 0, 0.2, 1) infinite;"></span>
                <span style="position: relative; width: 14px; height: 14px; border-radius: 50%; background: #E05A47; border: 2.5px solid #FFFFFF; box-shadow: 0 0 10px #E05A47;"></span>
              </div>
            `,
          ...
          touchdownMarkerRef.current = L.marker([lastFrame.latitude, lastFrame.longitude], {
            ...
            .bindTooltip(
              `<div style="font-family: monospace; font-size: 11px; padding: 2px 4px;">
                <strong style="color: #E05A47;">${label}</strong>
              </div>`,
